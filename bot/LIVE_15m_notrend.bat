@echo off
REM ===========================================================================
REM  SNRZ -- 15 minute chart, REAL ORDERS.
REM
...
setlocal
cd /d "%~dp0"
title SNRZ 15 minute chart  (trend filter OFF)

echo.
echo   SNRZ -- 15 minute chart  (trend filter OFF)
echo   ------------------------------------
...
REM      LIVE_15m_notrend.bat --symbol XAUUSD.m
python mt5_runner.py --tf 15 --no-trend --live %*
goto :done
