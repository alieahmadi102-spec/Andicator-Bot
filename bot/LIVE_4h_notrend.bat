@echo off
REM ===========================================================================
REM  SNRZ -- 4 hour chart, REAL ORDERS.
REM
...
setlocal
cd /d "%~dp0"
title SNRZ 4 hour chart  (trend filter OFF)

echo.
echo   SNRZ -- 4 hour chart  (trend filter OFF)
echo   ------------------------------------
...
REM      LIVE_4h_notrend.bat --symbol XAUUSD.m
python mt5_runner.py --tf 240 --no-trend --live %*
goto :done
