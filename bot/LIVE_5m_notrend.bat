@echo off
REM ===========================================================================
REM  SNRZ -- 5 minute chart, REAL ORDERS.
REM
...
setlocal
cd /d "%~dp0"
title SNRZ 5 minute chart  (trend filter OFF)

echo.
echo   SNRZ -- 5 minute chart  (trend filter OFF)
echo   ------------------------------------
...
REM      LIVE_5m_notrend.bat --symbol XAUUSD.m
python mt5_runner.py --tf 5 --no-trend --live %*
goto :done
