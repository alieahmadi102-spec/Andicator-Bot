@echo off
REM ===========================================================================
REM  SNRZ -- 1 minute chart, REAL ORDERS.
REM
...
setlocal
cd /d "%~dp0"
title SNRZ 1 minute chart  (trend filter OFF)

echo.
echo   SNRZ -- 1 minute chart  (trend filter OFF)
echo   ------------------------------------
...
REM      LIVE_1m_notrend.bat --symbol XAUUSD.m
python mt5_runner.py --tf 1 --no-trend --live %*
goto :done
